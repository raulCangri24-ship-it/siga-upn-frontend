...
function HorariosCoordinador() {
  ...
  const cargar = async () => {
    ...
    } catch { mostrarMsg('Error al cargar horarios', 'error') }
  }
  ...
  const seccionesFiltradas = secciones.filter(s =>
    `${s.curso} ${s.docente} ${s.codigo} ${s.horario}`.toLowerCase().includes(filtro.toLowerCase())
  )

  return (
    <PageShell role="coordinador" navTitle="Horarios del Periodo">
      <Alert message={mensaje} variant={tipoMensaje} onClose={() => setMensaje(null)} />

      <PageHeader
        title="Horarios del Periodo"
        subtitle={`Periodo 2026-1 · ${secciones.length} secciones · Vista de solo lectura`}
      />
      ...
      <div style={{ position: 'relative', maxWidth: '400px', marginBottom: '20px' }}>
        ...
        <input
          placeholder="Buscar por curso, docente, código u horario..."
          value={filtro}
              ...
              {seccionesFiltradas.length === 0 ? (
                <tr>
                  <td colSpan={7} style={{ padding: '48px', textAlign: 'center', color: 'var(--text-muted)', fontSize: '13px' }}>
                    No hay secciones registradas para este periodo
                  </td>
                </tr>
                ...
                return (
                  ...
                  >
                    ...
                    <td style={{ padding: '12px 16px', fontSize: '12px', color: 'var(--text-secondary)', whiteSpace: 'nowrap', fontWeight: '600' }}>{s.horario || '—'}</td>
                    <td style={{ padding: '12px 16px' }}>
                      <div style={{ fontSize: '12px', fontWeight: '600', color: lleno ? 'var(--danger-text)' : 'var(--success-text)', marginBottom: '4px' }}>
                    ...
                    <td style={{ padding: '12px 16px' }}>
                      <span style={{ padding: '3px 10px', borderRadius: '20px', fontSize: '11px', fontWeight: '700', background: lleno ? 'var(--danger-bg)' : 'var(--success-bg)', color: lleno ? 'var(--danger-text)' : 'var(--success-text)' }}>
                        {s.estado}
                      </span>
                    </td>
                  </motion.tr>
...
export default HorariosCoordinador
